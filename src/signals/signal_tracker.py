...
class RealSignalTracker:
    """
    🎯 Tracks all real signals generated by the system
    ...
    async def initialize(self):
        ...
        if not self.enabled:
            logger.info("⚠️ Signal tracking disabled in configuration")
            return
        ...
        try:
            ...
            logger.info("✅ Real Signal Tracker initialized")
            
        except Exception as e:
            logger.error(f"❌ Failed to initialize Signal Tracker: {e}")
            logger.error("💡 Make sure PostgreSQL is running and database exists")
            logger.error(f"💡 Database URL: {self.db_url}")
            self.enabled = False
        ...
        try:
            async with self.connection_pool.acquire() as conn:
                ...
                if os.path.exists(migration_path):
                    ...
                    logger.info("✅ Signal tracking tables ready")
                else:
                    # Create tables inline if migration file doesn't exist
                    await self._create_tables_inline(conn)
                
        except Exception as e:
            logger.error(f"❌ Failed to create signal tracking tables: {e}")
            raise
    ...
    async def _create_tables_inline(self, conn):
        ...
        """)
        logger.info("✅ Created signal tracking tables inline")
    ...
    async def log_signal(
        ...
        try:
            ...
            logger.info(f"📊 Logged signal: {record.symbol} {record.strategy} {record.direction} (ID: {signal_id[:8]}...)")
            return signal_id
    ...
    async def update_signal_outcome(
        ...
        try:
            ...
            async with self.connection_pool.acquire() as conn:
                ...
                if not signal_data:
                    logger.warning(f"⚠️ Signal {signal_id} not found for outcome update")
                    return
                ...
                logger.info(f"📊 Updated signal outcome: {signal_id[:8]}... -> {status} ({pnl_pct:.2%})")
